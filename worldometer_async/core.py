...
from .custom_html import HTML, AsyncHTMLSession, HTMLSession
...
class Worldometer(object):
    ...
    def __init__(self, timeout: int = 30, async_usage: bool = False):
        """Initializer of Worldometer class.

        Parameters
        ----------
        timeout
           Seconds of sleep for processing.
        """
        self.r = None  # Stores the response with html code for later rendering
        self.timeout = timeout
        self.async_usage = async_usage

        if not self.async_usage:
    ...
    async def __aenter__(self) -> "Worldometer":
        ...
        if getattr(self, "_metrics", None) is not None and not self.async_usage:
            ...
            self._metrics = await self.collect_metrics_async()
        return self

    async def __aexit__(self, exc_type, exc_value, traceback) -> None:
        """Exit of Worldometer class asynchronously."""
        if self.r and self.async_usage:
            await self.r.html.session.close()
    ...
    def metrics(self) -> list:
        ...
        -------
        list
            A list with all metrics of worldometer.
    ...
    async def _get_html_async(self, url: str) -> str:
        ...
        return its rendered content asynchronously.
        """

        session = AsyncHTMLSession()

        # Get html page and render dynamic content
        self.r: Response = await session.get(url, timeout=self.timeout)

        await self.r.html.arender(timeout=self.timeout, sleep=1)
        return self.r.html.raw_html
    ...
    def _get_html(self, url: str) -> str:
        ...
        try:
            # Get html page and render dynamic content
            self.r = session.get(url, timeout=self.timeout)
            self.r.html.render(timeout=self.timeout)
    ...
    def find_metrics_in_html(html_code: str) -> list:
        ...
        metrics = [
            metric.text for metric in html.find(_CSS_SELECTOR_OF_COUNTER_NUMBERS)
        ]
    ...
    async def collect_metrics_async(self) -> list:
        """Collects all metrics from the worldometer site asynchronously.
        ...
        if self.r is None:
            html = await self._get_html_async(url=URL)
        else:
            html = self.r.html.raw_html
    ...
    async def update_metrics_async(self) -> None:
        """Update metrics of worldometer asynchronously."""

        if self.r is not None:
            await self.r.html.arender(timeout=self.timeout)
            self._metrics = await self.collect_metrics_async()
        else:
    ...
    def what_is_here(self) -> dict:
        ...
        return {"categories": c, "labels": l, "metrics": m}
